...
python ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json"  --data-name-list "national_illness.csv" --strategy-args '{"pred_len":24}' --model-name "time_series_library.Triformer"   --model-hyper-params '{"d_model":32, "batch_size":32,"d_ff":64, "seq_len":96,"pred_len":96}'  --adapter "transformer_adapter"  --gpus 7  --num-workers 1 --timeout 6000  --save-path "for_validation7"
...
python ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json"  --data-name-list "national_illness.csv" --strategy-args '{"pred_len":24}' --model-name  "darts.TCNModel" --model-hyper-params  '{"n_epochs":100,"input_chunk_length":96,"output_chunk_length":24}'  --gpus 2  --num-workers 1 --timeout 60000  --save-path "for_validation7"

python ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json"  --data-name-list "national_illness.csv" "fred_md_dataset.csv" "nn5_daily_dataset_without_missing_values.csv"  --strategy-args '{"pred_len":60}' --model-name  "self_implementation.VAR_model" --gpus 6 --num-workers 1 --timeout 100000 --save-path "for_validation7"
